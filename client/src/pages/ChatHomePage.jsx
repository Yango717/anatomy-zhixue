...
import AutoPilotChatBox from '../components/ai/AutoPilotChatBox';
...
export default function ChatHomePage() {
  ...
  useEffect(() => {
    ...
    if (readThreadMessages(threadId).length > 0) return;
...
虽然我是AI驱动的，但你就当我是你身边一个热心的学姐就好～有什么想知道的，直接问我吧！`;
  ...
  const showTabs = autoPilotEnabled;
  const currentTab = showTabs ? activeHomeTab : 'chat';
  ...
  return (
    <div className="chat-home">
      ...
      <div className="chat-home__header">
        ...
        <div>
          <div className="chat-home__name">
            {currentTab === 'auto' ? '妍学姐 · 自主' : '妍学姐'}
          </div>
      ...
      {/* Tab Bar — only shown when AutoPilot is enabled */}
      {showTabs && (
        <div className="chat-home__tabs">
          <button
            className={`chat-home__tab ${currentTab === 'chat' ? 'chat-home__tab--active' : ''}`}
            onClick={() => switchHomeTab('chat')}
          >
            💬 对话学姐
          </button>
          <button
            className={`chat-home__tab ${currentTab === 'auto' ? 'chat-home__tab--active' : ''}`}
            onClick={() => switchHomeTab('auto')}
          >
            🧭 自主学姐
          </button>
        </div>
      ...
      {currentTab === 'auto' ? (
        ...
        /* ─── Chat Tab (original chat UI, plan-free) ─── */
        <>
          {/* Messages */}
